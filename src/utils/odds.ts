import { spellCards, type SpellCard, type SpellLevel } from './spells';
...
export function computeSpellOdds({
    ...
    const conjWeight = conjCards.reduce((s, c) => s + (levelWeights[c.level as SpellLevel] ?? 0) * c.weightMultiplier, 0);
    const stapleWeight = stapleCards.reduce((s, c) => s + (levelWeights[c.level as SpellLevel] ?? 0) * c.weightMultiplier, 0);
    ...
    return spellCards.map((spell) => {
        ...
        const pDraw = poolWeight > 0 ? pPool * (levelWeights[spell.level as SpellLevel] ?? 0) * spell.weightMultiplier / poolWeight : 0;
        ...
        return { spell, pDraw, pHitInPack, expectedPacks, goldNeeded };
